...
- On each Whisper **committed** segment, the app runs [`useLLM`](https://docs.swmansion.com/react-native-executorch/docs/hooks/natural-language-processing/useLLM) with the fine-tuned LFM2.5-350M `.pte` from Hugging Face.
...
- `App.tsx` — local SQLite + Face ID / PIN lock + tabs; wraps the tree in `OnDeviceAIProvider`.
- `utils/OnDeviceAIProvider.tsx` — loads `useLLM` with the Hugging Face fine-tuned invoice `.pte`.
